...
def write_seed(
    ...
    # clauses while keeping the total migration statement count bounded.
    for start in range(0, len(chunk_rows), 20):
        payload = json_text(chunk_rows[start : start + 20])
        statements.append(
            "INSERT INTO wiki_chunks "
            "(id,page_path,ordinal,heading_path,text,token_count,source_refs_json,content_sha) "
            ...
            f"FROM json_each({sql_string(payload)});"
        )

    for start in range(0, len(term_rows), 250):
        payload = json_text(
            [list(row) for row in term_rows[start : start + 250]]
        )
        statements.append(
